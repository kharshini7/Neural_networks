...
def build_generator():
    ...
    """
    model = tf.keras.models.Sequential([
        tf.keras.layers.Dense(7*7*8, use_bias=False, input_shape=(100,)),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.LeakyReLU(),
        tf.keras.layers.Reshape((7, 7, 8)),
        tf.keras.layers.Conv2DTranspose(8, (5, 5), strides=(1, 1), padding='same'),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.LeakyReLU(),
        tf.keras.layers.Conv2DTranspose(16, (5, 5), strides=(2, 2), padding='same'),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.LeakyReLU(),
...
class DCGAN(tf.keras.Model):
    ...
    def train_step(self, data):
        ...
        # Generate noise vector using a uniform distribution
            noise = tf.random.uniform([batch_size, 100])
            generated_images = self.generator(noise, training=True)
            real_output = self.discriminator(data, training=True)
            fake_output = self.discriminator(generated_images, training=True)
            g_loss = self.loss_fn(tf.ones_like(fake_output),fake_output)
            real_disc_loss = self.loss_fn(tf.ones_like(real_output), real_output)
            fake_disc_loss = self.loss_fn(tf.zeros_like(fake_output), fake_output)
            d_loss = 0.5 * (real_disc_loss + fake_disc_loss)
